...
namespace PROJETAKIP.Controllers
{
    ...
    {
        private ProjeTakipDBContext db = new ProjeTakipDBContext(); //Veri tabanımın bağlantısı


        public ActionResult Index()//veri listeleme
        {
            return View(db.PersonelBilgileris.ToList());
        }

        public ActionResult PersonelKart()//veri listeleme
        {
            return View(db.PersonelBilgileris.ToList());
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "PersonelBilgileriId,EPosta,Sifre,Yetki,AdSoyad,TCNO,Departman,Gorev,PozisyonAciklama,TelNO,Adres,MedeniHal,YakınBilgisi,YakınTC,YakınAdSoyad,YakınTel,DogumTarihi,IseGirisTarihi")] PersonelBilgileri personelBilgileri)
        {
            if (ModelState.IsValid)
            {
